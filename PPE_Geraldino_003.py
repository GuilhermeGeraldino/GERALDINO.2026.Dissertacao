...
# DEFINIÇÃO DE NOME PARA O ARQUIVO CONSOLIDADO DO ITEM 3.3 DA DISSERTAÇÃO
descricao = 'PPE_003'
...
def sim_model(tracao, ext_lt, coord_x, coord_y, model_name, job_name, raio, carga):
    ...
    mdb.models[model_name].DisplacementBC(
        ...
        u1=-1*(ext_lt)*math.cos(teta_ref), u2=(ext_lt)*math.sin(teta_ref), ur3=UNSET)
    ...
    for ang in angulos:
        ...
        for i in range(len(caso_lt)):
            ...
            for j in range(n_tracao):
                tracao = j * -1
                job_name = "3Job-{}_{}_{}_{}".format(diam, ang, caso_lt[i], f"{-tracao:03d}")
                model_name = '3Anl_{}_{}_{}_{}'.format(diam, ang, caso_lt[i], f"{-tracao:03d}")
